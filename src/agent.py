...
class Perception:
	...
	def _observe_state(self, room: Room, obj: RoomObject) -> list[Fact]:
		...
		if "locked" in obj.state:
			observed.extend(self._add_fact(Fact(obj.id, "is", "locked" if obj.state["locked"] else "unlocked")))

		if "open" in obj.state:
			observed.extend(self._add_fact(Fact(obj.id, "is", "open" if obj.state["open"] else "closed")))

		if "read" in obj.state:
			observed.extend(self._add_fact(Fact(obj.id, "is", "read" if obj.state["read"] else "unread")))
		...
		if obj.kind == "panel" and "needs_code" in obj.state:
			observed.extend(self._add_fact(Fact(obj.id, "needs_code", str(obj.state["needs_code"]))))

		if obj.kind == "lock" and "target" in obj.state:
			observed.extend(self._add_fact(Fact(obj.id, "is_on", str(obj.state["target"]))))
		...
		if obj.kind == "exit":
			for required_lock in sorted(obj.state.get("requires", [])):
				observed.extend(self._add_fact(Fact(obj.id, "requires_lock", required_lock)))
...
class Planner:
	...
	def select_action(self, kb: KnowledgeBase, room: Room, record: bool = True) -> Action | None:
		action = (
	...
	def _next_clue_action(self, kb: KnowledgeBase) -> Action | None:
		clue_ids = {fact.subject for fact in kb.query(predicate="should", object="be_examined")}
		if not clue_ids:
			available = {fact.subject for fact in kb.query(predicate="is", object="available")}
			unread = {fact.subject for fact in kb.query(predicate="is", object="unread")}
	...
	def _next_open_lock_action(self, kb: KnowledgeBase) -> Action | None:
		known_keys = {fact.subject for fact in kb.query(predicate="is", object="available")}
		candidate_locks = sorted(kb.query(predicate="uses_key"), key=lambda fact: fact.subject)
	...
	def _next_enter_code_action(self, kb: KnowledgeBase) -> Action | None:
		...
		for panel_fact in sorted(kb.query(predicate="needs_code"), key=lambda fact: fact.subject):
	...
	def _next_open_exit_action(self, kb: KnowledgeBase, room: Room) -> Action | None:
		for fact in sorted(kb.query(predicate="is", object="reachable"), key=lambda item: item.subject):
...
class ActionExecutor:
	...
	def execute(self, action: Action | None, room: Room, kb: KnowledgeBase) -> list[Fact]:
		if action is None:
			return []
	...
	def _execute_examine_clue(self, action: Action, room: Room, kb: KnowledgeBase) -> list[Fact]:
	...
	def _execute_open_exit(self, action: Action, room: Room, kb: KnowledgeBase) -> list[Fact]:
...
class EscapeRoomAgent:
	...
	def step(self) -> StepResult:
		...
		next_planned_action = self.planner.select_action(self.kb, self.room, record=False)
